"""gui/install_tab.py — 左侧分类列表 + 右侧大图标网格（微软商店风格）。
...
class InstallTab(ttk.Frame):
    ...
    def __init__(self, parent: tk.Widget, config: dict,
        ...
        self._batch_frame: Optional[ttk.Frame] = None
    ...
    def _build_ui(self) -> None:
        self._sidebar = ttk.Frame(self, width=160)
        self._sidebar.pack(side=tk.LEFT, fill=tk.Y, padx=(4, 0), pady=4)
        self._sidebar.pack_propagate(False)

        ttk.Label(self._sidebar, text="分类", font=("Microsoft YaHei UI", 10, "bold")).pack(
            anchor=tk.W, padx=8, pady=(4, 4)
        )
        ...
        self._cat_listbox.pack(fill=tk.BOTH, expand=True, padx=4, pady=2)
        ...
        ttk.Button(top_bar, text="🔄 刷新列表", command=self.refresh).pack(side=tk.RIGHT)
        ttk.Button(top_bar, text="➕ 导入安装包", command=self._import_installer).pack(side=tk.RIGHT, padx=4)
        ttk.Button(top_bar, text="📁 分类管理", command=self._new_category).pack(side=tk.RIGHT, padx=4)
        
        self._batch_mode_btn = ttk.Checkbutton(
            top_bar, text="批量选择", variable=self._batch_mode_var,
            command=self._on_batch_mode_toggle
        )
        self._batch_mode_btn.pack(side=tk.RIGHT, padx=4)

        content_frame = ttk.Frame(right)
        content_frame.pack(fill=tk.BOTH, expand=True)

        self._canvas = tk.Canvas(content_frame, highlightthickness=0, bg="#F5F5F5")
        ...
        self._batch_frame = ttk.Frame(right, relief=tk.RIDGE, borderwidth=1)
    ...
    def _new_category(self) -> None:
        tools_dir = self._resolve_tools_dir()
        dialog = CategoryManageDialog(self, tools_dir)
    ...
    def _update_batch_frame(self) -> None:
        ...
        row = ttk.Frame(self._batch_frame)
        row.pack(fill=tk.X, pady=4)

        if self._selected_tools:
            ttk.Label(row, text=f"📦 批量选择模式 - 已选择 {len(self._selected_tools)} 个工具",
                      font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT, padx=8)
        else:
            ttk.Label(row, text="📦 批量选择模式 - 请选择工具",
                      font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT, padx=8)
        ...
        if self._selected_cat:
            tools = {
                ...
                if self._selected_cat in v["categories"]
            }
        all_selected = len(self._selected_tools) == len(tools) and len(tools) > 0

        ttk.Button(row, text="删除选中",
                   command=self._batch_remove_tools).pack(side=tk.RIGHT, padx=4)
        ...
        ttk.Button(row, text="取消选择",
                   command=self._clear_selection).pack(side=tk.RIGHT, padx=4)
        ttk.Button(row, text="分类到...",
                   command=self._batch_categorize).pack(side=tk.RIGHT, padx=4)
    ...
    def _batch_categorize(self) -> None:
        ...
        dialog = _BatchCategorizeDialog(self, cat_options)
        self.wait_window(dialog)
    ...
    def _rebuild_cat_list(self) -> None:
        ...
        for cat in self._scan["categories"]:
            count = sum(
                1 for t in self._scan["tools"].values()
                if cat["code"] in t["categories"]
            )
    ...
    def _rebuild_cards(self) -> None:
        ...
        if not tools:
            ttk.Label(self._grid_frame, text="该分类下没有工具",
                      font=("Microsoft YaHei UI", 11)).grid(row=0, column=0, padx=20, pady=20)
            return
        ...
        for key in sorted(tools.keys()):
            ...
            card.grid(row=row, column=col, padx=CARD_PAD_X // 2, pady=CARD_PAD_Y // 2,
                      sticky=tk.N)
    ...
    def _make_card(self, parent: tk.Widget, key: str, tool: ToolInfo,
                   state: dict) -> ttk.Frame:
        card = ttk.Frame(parent, width=CARD_W, height=CARD_H, relief=tk.GROOVE, borderwidth=1)
        ...
        try:
            ...
            icon_label = ttk.Label(card, image=img)
        except tk.TclError:
            icon_label = ttk.Label(card, text="📦", font=("Segoe UI Emoji", 24))
        icon_label.pack(pady=(10, 4))

        remove_btn = ttk.Button(card, text="✕", width=2,
                                command=lambda k=key, t=tool: self._on_remove_tool(k, t))
        remove_btn.place(x=CARD_W - 24, y=4)

        name = tool.get("name") or tool["folder_name"]
        ttk.Label(card, text=name, font=("Microsoft YaHei UI", 9, "bold"),
                  wraplength=CARD_W - 16, justify=tk.CENTER).pack(pady=(0, 2))

        installers = tool.get("installers", [])
        version_var = tk.IntVar(value=0)

        if len(installers) > 1:
            ver_frame = ttk.Frame(card)
            ver_frame.pack(pady=2)
            labels = [inst["label"] for inst in installers]
            combo = ttk.Combobox(ver_frame, values=labels, state="readonly",
                                 width=16, font=("Microsoft YaHei UI", 8))
            combo.current(0)
            combo.pack()
            ...
            if ver:
                ttk.Label(card, text=f"v{ver}", font=("Microsoft YaHei UI", 8),
        ...
        btn_state = tk.DISABLED if installed or not installers else tk.NORMAL

        btn = ttk.Button(card, text=btn_text, state=btn_state,
                         command=lambda k=key, t=tool, vv=version_var: self._on_install(k, t, vv))
        btn.pack(pady=(6, 8))
    ...
    def _handle_install_status(self, key: str, status: str, detail: str) -> None:
        if status == "installed":
            ...
            self._set_status(f"正在启动安装程序...")
        elif status == "polling":
            self._set_status(f"等待安装完成...")
